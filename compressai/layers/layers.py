    ...
    def forward(self, x):
        if self.use_layer_scale:
            ...
            if self.index% 2==0:
                ...
                anchor = anchor + self.drop_path(self.token_mixer_an(self.norm1_an(anchor)))
                anchor = anchor + self.drop_path(self.mlp_an(self.norm2_an(anchor)))
                non_anchor = non_anchor + self.drop_path(self.token_mixer_nonan(self.norm1_nonan(non_anchor)))
                non_anchor = non_anchor + self.drop_path(self.mlp_nonan(self.norm2_nonan(non_anchor)))
                x = anchor+non_anchor
