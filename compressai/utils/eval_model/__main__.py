...
def inference(model, x, filename, recon_path):
    ...
    start = time.time()
    out_dec = model.decompress(out_enc["strings"], out_enc["shape"],out_enc["latent_feature"],out_enc["y"])
    ...
    # print(len(out_enc["strings"][0][0]))
    bpp = out_enc["string_lenth"]* 8.0 / num_pixels
    # bpp = sum(len(s[0]) for s in out_enc["strings"]) * 8.0 / num_pixels
    # print(psnr(x, out_dec["x_hat"]))
    ssim= ms_ssim(x, out_dec["x_hat"], data_range=1.0).item()
